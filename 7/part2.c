...
int getScore(hand* h) {
    ...
    char mostCommonCard = 'A';
    for (int i = 0; i < 5; i++) {
        if (h->cards[i] == 'J') continue;
        if (counts[h->cards[i]] > mostCommonCount) {
            mostCommonCard = h->cards[i];
            mostCommonCount = counts[h->cards[i]];
        }
    }
    counts[mostCommonCard] += counts['J'];
    counts['J'] = 0;
    ...
    for (int i = 0; i < 4; i++) {
        if (cardSorted[i] != cardSorted[i+1]) {
            isFiveOfAKind = false;
            break;
        }
    }
